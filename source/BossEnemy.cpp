...
BossEnemy::BossEnemy(const sf::FloatRect& platformCollider, const std::string& projectPath)
    ...
    currentFrame = { 0, static_cast<int>(frameHeight) * rowIndex, static_cast<int>(frameWidth), static_cast<int>(frameHeight) };
    ...
    float marginX = 10.f;
    float marginY = 12.f;
    shape.setSize({ spriteBounds.width - 2.f * marginX, spriteBounds.height - 2.f * marginY });
    shape.setPosition({ spriteBounds.left + marginX, spriteBounds.top + marginY });
...
void BossEnemy::updateAnimation(float deltaTime) {
    ...
    if (animationTimer >= animationSpeed) {
        ...
        currentFrame.top = static_cast<int>(frameHeight) * rowIndex;
